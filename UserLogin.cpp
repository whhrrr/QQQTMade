...
bool UserLogin::veryfyAccountCode(bool& isAccountLogin, QString& strAccount)
{
	...
	if (queryEmployeeID.first())//指向结果集第一条
	{
		...
		if (strCode == strCodeInput)
		{
			...
			isAccountLogin = false;//确认是否为账号登录
			strAccount = strAccountInput;//保存登陆的账号
			return true;
		}
		else {
	...
	if (queryAccount.first())
	{
		...
		if (strCode == strCodeInput) 
		{
			...
			isAccountLogin = true;//确认是否为帐号登录
			strAccount = strAccountInput;//保存输入的账号
			return true;
		}
		else {
			return false;
...
void UserLogin::onLoginBtnClicked() 
{
	...
	{
		QMessageBox::information(NULL, QString::fromLocal8Bit("账号密码有误"),
